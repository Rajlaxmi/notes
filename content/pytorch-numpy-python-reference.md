...
> **Under the hood.** Broadcasting isn't a Python-level trick: both libraries push it into compiled code that computes the output shape, then iterate without copying.
>
> **NumPy** (C, `numpy/core/src/multiarray/`): `PyArray_Broadcast` builds a `PyArrayMultiIterObject` from per-array `PyArrayIterObject`s. To stretch a size-1 axis to size `N`, it sets that iterator's *stride to 0* for the axis instead of allocating anything, so the same data pointer gets revisited `N` times as the axis is walked. A shape that can't be aligned raises `ValueError: operands could not be broadcast together`. ([NumPy C code explanations](https://numpy.org/doc/stable/dev/internals.code-explanations.html))
>
> ```c
> /* simplified sketch of the stride-0 trick */
> if (dim_size == 1 && broadcast_size > 1) {
>     iter->strides[axis] = 0;   /* pointer never advances on this axis */
>     iter->dims_m1[axis] = broadcast_size - 1;
> }
> ```
>
> **PyTorch** (C++, `aten/src/ATen/`): shape inference and iteration are two separate steps. `infer_size_impl` in `ExpandUtils.cpp` walks both shapes from the trailing dimension backward, treating a missing dimension as `1`, and picks the non-1 size at each position, or throws `"The size of tensor a (...) must match the size of tensor b (...) at non-singleton dimension ..."` if both sides are non-1 and unequal. `TensorIterator::compute_shape()` (`TensorIterator.cpp`) calls this for every pointwise/reduction op and also handles dtype promotion, exposing the same expand-without-copy idea at the Python level via `Tensor.expand()`. ([ExpandUtils.cpp](https://github.com/pytorch/pytorch/blob/main/aten/src/ATen/ExpandUtils.cpp) · [TensorIterator.cpp](https://github.com/pytorch/pytorch/blob/main/aten/src/ATen/TensorIterator.cpp) · [Broadcasting semantics](https://github.com/pytorch/pytorch/blob/main/docs/source/notes/broadcasting.md))
>
> ```cpp
> // simplified sketch of infer_size_impl's core loop
> for (int64_t i = 1; i <= ndim; ++i) {
>   int64_t a = (i <= a_ndim) ? shape_a[a_ndim - i] : 1;
>   int64_t b = (i <= b_ndim) ? shape_b[b_ndim - i] : 1;
>   TORCH_CHECK(a == b || a == 1 || b == 1, "size mismatch at non-singleton dimension");
>   result[ndim - i] = (a == 1) ? b : a;
> }
> ```
>
